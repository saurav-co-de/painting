...
export function validateLineItemNumbers(items = []) {
  for (const [index, item] of items.entries()) {
    if (!isNonNegativeDecimal(item.quantity)) {
      return `Quantity for item ${index + 1} must be a non-negative number.`;
    }

    if (!isNonNegativeDecimal(item.rate)) {
      return `Rate for item ${index + 1} must be a non-negative number.`;
    }
  }
...
export function calculateInvoice(items = [], taxMode = "intra", options = {}) {
  const isWithoutGst = taxMode === "none";
  const isInterState = taxMode === "inter";
  const useDirectAmount = Boolean(options.useDirectAmount);
  const includeAmountOnlyItems = Boolean(options.includeAmountOnlyItems);
  const normalizedItems = items
    .filter((item) => {
      const hasDescription = Boolean(item.description?.trim());
      const hasAmount = String(item.amount ?? "").trim() !== "";
      const hasRate = String(item.rate ?? "").trim() !== "";

      return hasDescription || (includeAmountOnlyItems && (hasAmount || hasRate));
    })
    .map((item) => {
      const quantity = toNonNegativeNumber(item.quantity);
      const rate = toNonNegativeNumber(item.rate);
      const gstPercentage = isWithoutGst ? 0 : toNonNegativeNumber(item.gstPercentage);
      const directAmount = toFiniteNumber(item.amount);
      const hasDirectAmount = String(item.amount ?? "").trim() !== "";
      const amount = roundCurrency(useDirectAmount && hasDirectAmount ? directAmount : quantity * rate);
