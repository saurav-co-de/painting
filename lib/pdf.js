...
export function generateInvoicePdf(invoice, options = {}) {
  ...
  let y = 665;
  if (invoice.customerDetails.clientName) {
    lines.push(textLine(invoice.customerDetails.clientName, contentLeft, y, 9, "F2"));
    y -= 13;
  }
  wrapText(invoice.customerDetails.address, 38)
  ...
  lines.push(...underlinedCenteredText(`${documentNumberLabel}: ${documentNumber}`, contentLeft, 694, contentWidth, 9, "F2"));
  ...
  lines.push(...underlinedCenteredText(`Sub : ${subjectPrefix} ${billSubject}`, contentLeft, subjectY, contentWidth, 12, "F2"));
  ...
  visibleItems.forEach((item, index) => {
    ...
    x += columnWidths[3];
    lines.push(rightTextFit(options.preserveRateText ? item.rate : money(item.rate), x + 3, rowY, columnWidths[4] - 6, 8, "F1"));
    x += columnWidths[4];
