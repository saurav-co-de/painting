...
export default function AppShell({ user, title, description, children, actions }) {
  ...
  return (
    <main className="min-h-screen min-w-0 bg-[var(--surface)] px-2.5 py-2.5 sm:px-5 xl:px-7 2xl:px-10 print:bg-white print:p-0">
          ...
          <aside className="glass-card sticky top-4 hidden h-[calc(100vh-2rem)] flex-col gap-6 overflow-y-auto p-5 xl:flex print:hidden">
            <div>
              <div className="flex items-center gap-3">
                <div className="flex h-12 w-12 items-center justify-center rounded-lg bg-slate-950 text-lg font-bold text-white">
                  {user.logoText || "BB"}
                </div>
                <div className="min-w-0">
              ...
              <div className="mt-6 rounded-lg border border-slate-200/80 bg-slate-50 p-4">
            ...
            <nav className="space-y-2">
              {navigation.map((item) => {
                ...
                return (
                  <Link
                    className={`group flex items-center justify-between rounded-lg px-4 py-3 text-sm font-semibold transition ${
                      isActive
            ...
            <header className="glass-card p-4 sm:p-7 lg:p-8 print:hidden">
              <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
                <div className="min-w-0">
                  ...
                  <h2 className="font-display mt-3 break-words text-2xl text-slate-950 sm:text-4xl xl:text-[2.75rem]">
                    {title}
                  </h2>
                ...
                {actions ? <div className="flex w-full min-w-0 flex-col gap-3 sm:flex-row sm:flex-wrap lg:w-auto lg:justify-end">{actions}</div> : null}
              </div>
            </header>
