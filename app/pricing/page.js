      ...
      <main className="mx-auto w-full max-w-[1800px] px-3 py-5 sm:px-6 sm:py-10 xl:px-8">
        <section className="glass-card p-5 sm:p-10">
          ...
          <div className="mt-8 grid gap-5 lg:grid-cols-3">
            {plans.map((plan) => (
              <article className="rounded-xl border border-slate-200/80 bg-white/85 p-5 sm:p-6" key={plan.name}>
                ...
                <div className="mt-6 space-y-3">
                  {plan.features.map((feature) => (
                    <div className="rounded-xl bg-[var(--surface)] px-4 py-3 text-sm text-slate-700" key={feature}>
                      {feature}
                    </div>
                  ))}
                </div>
